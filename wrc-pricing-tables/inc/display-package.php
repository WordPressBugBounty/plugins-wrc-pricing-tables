...
function wrcpt_view_pricing_packages()
{
	...
	if (!wp_verify_nonce($nonce, 'wrcpt_ajax_action_nonce')) {
		...
		<div id="tabledisplaydiv">
			<h3><span id="editPackages" class="button button-large"
					onclick="wrcpteditpackages(<?php echo esc_attr($packageCount); ?>, '<?php echo esc_attr($pricing_table); ?>')"><?php esc_html_e('Edit Columns', 'wrc-pricing-tables'); ?></span>
			</h3>
