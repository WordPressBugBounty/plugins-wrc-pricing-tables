...
if (!class_exists('WRCPT_Process_Columns')) {
	class WRCPT_Process_Columns
	{
		private static $instance;

		public function __construct()
		{
			add_action('wp_ajax_nopriv_wrcpt_edit_pricing_packages', array($this, 'wrcpt_edit_pricing_packages'));
			add_action('wp_ajax_wrcpt_edit_pricing_packages', array($this, 'wrcpt_edit_pricing_packages'));
		}
		...
		/**
		 * Renders the complete pricing package (column) editor UI for a pricing table.
		 *
		 * This method is responsible for displaying and preparing all editable fields
		 * required to manage individual pricing table columns, including:
		 *
		 * - Editing package titles, prices, buttons, ribbons, descriptions, and all
		 *   column-specific values entered by the user.
		 * - Displaying and updating feature values for each column, based on the
		 *   features previously created in the “Edit Features” section.
		 * - Allowing the admin to add new pricing columns dynamically.
		 * - Rendering sortable columns so the user can reorder packages by dragging.
		 * - Outputting ColorPicker-enabled fields for customizing each column’s
		 *   background, text color, hover color, and other visual settings.
		 * - Rendering global table configuration fields defined inside the
		 *   “Pricing Table Settings” panel, allowing users to style and control
		 *   the entire table layout.
		 *
		 * This method only generates the interface markup needed for AJAX loading.
		 * Saving or processing column data is handled separately inside the
		 * WRCPT_Process_Options class.
		 *
		 * Execution ends with `wp_die()` because the method is used as an AJAX endpoint.
		 * 
		 * @return void
		 */
		public function wrcpt_edit_pricing_packages()
		{
			...
			if (!wp_verify_nonce($nonce, 'wrcpt_ajax_action_nonce')) {
					...
					<div class="tablecolumnwrap">
						...
						<div id="addButtons">
							<a id="addPackage" class="newcolumn" href="#" role="button">
								<span><?php esc_html_e('New Column', 'wrc-pricing-tables'); ?></span>
								<div class="icon">
						...
						<div class="accordion-expand-holder">
							<button type="button" class="expand"><span class="dashicons dashicons-editor-expand"></span>&nbsp;<?php esc_html_e(' Expand', 'wrc-pricing-tables'); ?></button>
							<button type="button" class="collapse"><span class="dashicons dashicons-fullscreen-exit-alt"></span>&nbsp;<?php esc_html_e(' Collapse', 'wrc-pricing-tables'); ?></button>
						</div>
						<div id="sortable_column">
							...
							if (!empty($package_lists)) {
								foreach ($packageOptions as $option => $value) {
									...
									if (isset($packageValue['pdisp'])) {
										$showpack = $packageValue['pdisp'];
									} else {
										$showpack = 'show';
									} ?>
									<div id="wrcpt-<?php echo esc_attr($ac); ?>" class="package_details">
										<h4 id="pcolumn<?php echo esc_attr($ac); ?>" class="column_count">
											<?php esc_html_e('Pricing Column', 'wrc-pricing-tables'); ?>
											<?php echo esc_attr($ac); ?>
										</h4>
										<?php if ($packageValue['pdisp'] == 'show') { ?><span id="hidePack<?php echo esc_attr($ac); ?>" class="column_hide"><span class="dashicons dashicons-fullscreen-alt"></span><?php } else { ?><span id="showPack<?php echo esc_attr($ac); ?>" class="column_show"><span class="dashicons dashicons-fullscreen-exit-alt"></span><?php } ?><input type="hidden" name="hide_show[]" value="<?php echo esc_attr($packageValue['pdisp']); ?>" /></span>
											...
											<div id="accordion<?php echo esc_attr($ac); ?>" class="column_container">
												...
												<div class="element-input">
													...
													<input type="checkbox" class="tickbox" name="special_package[<?php echo esc_attr($ac - 1); ?>]" id="special_package" value="yes" <?php if ($packageValue['spack'] == 'yes') { ?> checked="checked" <?php } ?> />
													<hr />
													...
													<textarea name="package_desc[]" class="medium" id="package_desc" cols="27" rows="2" placeholder="<?php esc_html_e('Enter Short Description', 'wrc-pricing-tables'); ?>"><?php echo esc_attr($packageValue['tdesc']); ?></textarea>
													<hr />
													...
													<textarea name="price_desc[]" class="medium" id="price_desc" cols="27" rows="2" placeholder="<?php esc_html_e('Enter Short Description', 'wrc-pricing-tables'); ?>"><?php echo esc_attr($packageValue['pdesc']); ?></textarea>
													<hr />
													...
													if ($package_feature) {
														...
														echo '<label class="input-title">' . esc_html_e('There are no feature items!', 'wrc-pricing-tables') . '</label>';
													} ?>
								...
								esc_html_e('No Packages yet!', 'wrc-pricing-tables');
							} ?>
						</div> <!--sortable_column -->
					</div> <!--tablecolumnwrap -->
				</div> <!--tablecolumndiv -->
						...
						<div id="accordion_advance" class="package_advance">
							...
							<div class="advance-input">
								<label class="input-check">
									<?php esc_html_e('Enable Pricing Table', 'wrc-pricing-tables'); ?>:
									<input type="checkbox" name="wrcpt_option" class="tickbox" id="wrcpt_option" value="yes" <?php if ($packageCombine['enable'] == 'yes') { ?> checked="checked" <?php } ?> />
								</label>
								...
								<label class="input-check">
									<?php esc_html_e('Enable Column Shadow', 'wrc-pricing-tables'); ?>:
									<input type="checkbox" name="column_shadow" class="tickbox" id="column_shadow" value="yes" <?php if ($packageCombine['colshad'] == 'yes') { ?> checked="checked" <?php } ?> />
								</label>
								<label class="input-check">
									<?php esc_html_e('Disable Shadow on Highlight', 'wrc-pricing-tables'); ?>:
									<input type="checkbox" name="disable_shadow" class="tickbox" id="disable_shadow" value="yes" <?php if ($packageCombine['dscol'] == 'yes') { ?> checked="checked" <?php } ?> />
								</label>
								<label class="input-check">
									<?php esc_html_e('Disable Title Gradient', 'wrc-pricing-tables'); ?>:
									<input type="checkbox" name="title_gradient" class="tickbox" id="title_gradient" value="yes" <?php if ($packageCombine['ttgrd'] == 'yes') { ?> checked="checked" <?php } ?> />
								</label>
								<label class="input-check">
									<?php esc_html_e('Align Price Unit at Right', 'wrc-pricing-tables'); ?>:
									<input type="checkbox" name="unit_right" class="tickbox" id="unit_right" value="yes" <?php if ($packageCombine['purgt'] == 'yes') { ?> checked="checked" <?php } ?> />
								</label>
								<label class="input-check">
									<?php esc_html_e('Open Link in New Tab', 'wrc-pricing-tables'); ?>:
									<input type="checkbox" name="new_tab" class="tickbox" id="new_tab" value="yes" <?php if ($packageCombine['nltab'] == 'yes') { ?> checked="checked" <?php } ?> />
								</label>
								<label class="input-check">
									<?php esc_html_e('Enable Feature Tooltips', 'wrc-pricing-tables'); ?>:
									<input type="checkbox" name="enable_tooltip" class="tickbox" id="enable_tooltip" value="yes" <?php if ($packageCombine['entips'] == 'yes') { ?> checked="checked" <?php } ?> />
								</label>
								<div class="break-line"></div>
								<label class="input-check">
									<?php esc_html_e('Enable Package Ribbons', 'wrc-pricing-tables'); ?>:
									<input type="checkbox" name="enable_ribbon" class="tickbox" id="enable_ribbon" value="yes" <?php if ($packageCombine['enribs'] == 'yes') { ?> checked="checked" <?php } ?> />
								</label>
								<div class="break-line"></div>
							...
							<div class="advance-input">
								...
								<input type="text" name="tooltip_width" class="medium" id="tooltip_width" value="<?php echo esc_attr($packageCombine['ttwidth']); ?>" placeholder="e.g. 130px" />
								<hr />
				...
				<div class="wrcpt-clear"></div>
				<input type="hidden" id="submitted" name="submitted" value="<?php if (isset($packageCombine['subform']) && $packageCombine['subform']) {
					echo esc_attr($packageCombine['subform']);
				} else {
					echo 'no';
				} ?>" />
				...
				<button type="submit" id="wrcpt_edit" class="button-primary"><?php esc_html_e('Update Package', 'wrc-pricing-tables'); ?></button>
				<?php
			}
			wp_die();
		}
...
WRCPT_Process_Columns::get_instances();
