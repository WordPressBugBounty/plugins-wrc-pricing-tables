<?php
/**
 * Handles all processing and database operations for the WRC Pricing Tables plugin.
 *
 * This class is responsible for:
 * - Managing pricing table creation, updates, and deletion.
 * - Handling package options and regenerating related shortcode IDs.
 * - Updating feature lists for pricing packages.
 * - Cleaning up unused or orphaned package options.
 * - Ensuring all operations follow WordPress security practices including
 *   capability checks, nonce verification, and sanitized input handling.
 *
...
if (!class_exists('WRCPT_Process_Options')) {
	class WRCPT_Process_Options
	{
		private static $instance;

		public function __construct()
		{
			// Adding pricing tables features
			add_action( 'wp_ajax_nopriv_wrcpt_add_package_features', array( $this, 'wrcpt_add_package_features' ) );
			add_action( 'wp_ajax_wrcpt_add_package_features', array( $this, 'wrcpt_add_package_features' ) );

			// Updating pricing tables features
			add_action( 'wp_ajax_nopriv_wrcpt_update_package_features', array( $this, 'wrcpt_update_package_features' ) );
			add_action( 'wp_ajax_wrcpt_update_package_features', array( $this, 'wrcpt_update_package_features' ) );

			// Deleting a pricing table
			add_action( 'wp_ajax_nopriv_wrcpt_delete_pricing_table', array( $this, 'wrcpt_delete_pricing_table' ) );
			add_action( 'wp_ajax_wrcpt_delete_pricing_table', array( $this, 'wrcpt_delete_pricing_table' ) );

			// Updating pricing columns
			add_action( 'wp_ajax_nopriv_wrcpt_update_pricing_package', array( $this, 'wrcpt_update_pricing_package' ) );
			add_action( 'wp_ajax_wrcpt_update_pricing_package', array( $this, 'wrcpt_update_pricing_package' ) );

			// Regenerate Shortcode IDs
			add_action( 'wp_ajax_nopriv_wrcpt_regenerate_shortcode', array( $this, 'wrcpt_regenerate_shortcode' ) );
			add_action( 'wp_ajax_wrcpt_regenerate_shortcode', array( $this, 'wrcpt_regenerate_shortcode' ) );

			// Remove unnecessary package options
			add_action( 'wp_ajax_nopriv_wrcpt_unuseful_package_options', array( $this, 'wrcpt_unuseful_package_options' ) );
			add_action( 'wp_ajax_wrcpt_unuseful_package_options', array( $this, 'wrcpt_unuseful_package_options' ) );
		}
		 ...
		 * @since 1.0.0
		 */
		public function wrcpt_check_permissions_and_nonce() {
		...
		/**
		 * Handles creating pricing table package features via AJAX.
		 * 
		 * @return void
		 */
		public function wrcpt_add_package_features() {
			// Check permissions and nonce
			if ($this->wrcpt_check_permissions_and_nonce()) {
				$fn = 1;
		...
		/**
		 * Handles updating pricing table package features via AJAX.
		 * 
		 * @return void
		 */
		public function wrcpt_update_package_features() {
			// Check permissions and nonce
			if($this->wrcpt_check_permissions_and_nonce()) {
				$fn = 1; $count_item = 0;
		...
		/**
		 * Update pricing table name if changed
		 * 
		 * @param mixed $edited_table
		 * @param mixed $pricing_table
		 */
		public function wrcpt_edit_pricing_table($edited_table, $pricing_table) {
			// Check if the user has the necessary capability (e.g., manage_options)
			if (!current_user_can('manage_options')) {
		 ...
		 * @return void
		 */
		public function wrcpt_delete_pricing_table() {
			// Check permissions and nonce
			if($this->wrcpt_check_permissions_and_nonce()) {
		...
		/**
		 * Updates or adds a new package option entry inside a pricing table.
		 *
		 * @param mixed $pricing_table	The option key where the package list is stored.
		 * @param mixed $package_lists	Existing comma-separated package option names.
		 */
		...
		/**
		 * Deletes pricing package options that no longer exist in the updated package list.
		 *
		 * This method:
		 * - Compares the existing package list with the new list provided.
		 * - Deletes any package option (e.g., packageOptions3, packageOptions7) that
		 *   is present in the old list but missing from the new one.
		 *
		 * Important:
		 * - This function only removes option entries; it does not modify the main
		 *   pricing table list itself.
		 *
		 * @param mixed $pricing_table	The option key containing the stored list of package option names.
		 * @param mixed $new_lists		The updated, sanitized list of package option names to keep.
		 * 
		 * @return void
		 */
		public function wrcpt_delete_pricing_packages($pricing_table, $new_lists) {
			// Check if the user has the necessary capability (e.g., manage_options)
			if (!current_user_can('manage_options')) {
				// If the user does not have the required capability, terminate and display an error message.
				wp_die(esc_html__('You do not have sufficient permissions to access this page.', 'wrc-pricing-tables'));
		...
		/**
		 * Updates all pricing table settings and package data.
		 *
		 * Handles:
		 * - Permission + nonce validation
		 * - Sanitizing and saving table-level options
		 * - Updating or creating package option entries
		 * - Reordering packages and storing feature/tooltip values
		 * - Removing deleted packages from the database
		 *
		 * @return void
		 */
		public function wrcpt_update_pricing_package() {
			// Check permissions and nonce
			if($this->wrcpt_check_permissions_and_nonce()) {
				...
				$pricing_table = $this->wrcpt_edit_pricing_table($pricing_table, $pricing_table_name);
				...
				if (isset($_POST['pricing_packages'])) {
					...
					$this->wrcpt_delete_pricing_packages($pricing_table, $sanitized_pricing_packages);
				} else {
					$this->wrcpt_delete_pricing_packages($pricing_table, ''); // Provide a default value if $_POST['pricing_packages'] is not set
				}
		...
		/**
		 * Regenerates the shortcode IDs for all pricing tables.
		 * 
		 * @return void
		 */
		public function wrcpt_regenerate_shortcode() {
		...
		/**
		 * Removes unused package option entries from the database.
		 * 
		 * Responsibilities:
		 * - Collects all package options currently used by existing pricing tables.
		 * - Iterates through a predefined range of `packageOptions1–500` to detect
		 * unused or orphaned package option entries.
		 * - Deletes any package option that is not currently associated with
		 * any pricing table.
		 * - Returns a success response through AJAX with the number of deleted entries.
		 * 
		 * @return void
		 */
		public function wrcpt_unuseful_package_options() {
			// Check if the current user has the necessary capability
			if (!current_user_can('manage_options')) {
				wp_die(esc_html__('You do not have sufficient permissions to access this page.', 'wrc-pricing-tables'));
			}
			...
			* if any unuseful package exist or not */
			...
			if ($pcount > 0) {
				wp_send_json_success(
					array(
						'message' => 'Hurray! ' .esc_attr($pcount). ' unnecessary package options have been cleared!',
					)
				);
			} else {
				wp_send_json_success(
					array(
						'message' => 'Hurray! You don\'t have anymore unnecessary package options to clear!',
					)
				);
			}
...
WRCPT_Process_Options::get_instances();
